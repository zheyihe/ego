import { Agent, type AgentEvent } from "@zheyihe/ego-agent-core";
import { getModel, type ImageContent } from "@zheyihe/ego-ai";
import {
	AgentSession,
	AuthStorage,
	convertToLlm,
	...
	if (!key) {
		throw new Error(
			"No API key found for anthropic.\n\n" +
				...
				join(homedir(), ".ego", "mom", "auth.json"),
		);
	}
	return key;
}
...
function createRunner(sandboxConfig: SandboxConfig, channelId: string, channelDir: string): AgentRunner {
	...
	const authStorage = AuthStorage.create(join(homedir(), ".ego", "mom", "auth.json"));
