...
```bash
npm install @zheyihe/ego-ai
```

TypeBox exports are re-exported from `@zheyihe/ego-ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@zheyihe/ego-ai';
...
```typescript
import { Type, Tool, StringEnum } from '@zheyihe/ego-ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@zheyihe/ego-ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@zheyihe/ego-ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@zheyihe/ego-ai';
...
```typescript
import { getModel, stream } from '@zheyihe/ego-ai';
...
```typescript
import { getProviders, getModels, getModel } from '@zheyihe/ego-ai';
...
```typescript
import { Model, stream } from '@zheyihe/ego-ai';
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@zheyihe/ego-ai';
...
```typescript
import { getModel, complete, Context } from '@zheyihe/ego-ai';
...
```typescript
import { Context, getModel, complete } from '@zheyihe/ego-ai';
...
- OAuth login flows are not supported in browser environments. Use the `@zheyihe/ego-ai/oauth` entry point in Node.js.
...
Set `EGO_AI_ANTIGRAVITY_VERSION` to override the Antigravity User-Agent version when Google updates their requirements:

```bash
export EGO_AI_ANTIGRAVITY_VERSION="1.23.0"
```

#### Cache Retention

Set `EGO_CACHE_RETENTION=long` to extend prompt cache retention:

| Provider | Default | With `EGO_CACHE_RETENTION=long` |
|----------|---------|-------------------------------|
...
```typescript
import { getEnvApiKey } from '@zheyihe/ego-ai';
...
```bash
npx @zheyihe/ego-ai login              # interactive provider selection
npx @zheyihe/ego-ai login anthropic    # login to specific provider
npx @zheyihe/ego-ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@zheyihe/ego-ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from '@zheyihe/ego-ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from '@zheyihe/ego-ai/oauth';
...
```typescript
import { getModel, complete } from '@zheyihe/ego-ai';
import { getOAuthApiKey } from '@zheyihe/ego-ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@zheyihe/ego-ai`
