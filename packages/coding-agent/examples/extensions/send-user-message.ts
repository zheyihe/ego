 ...
 *
 * Demonstrates ego.sendUserMessage() for sending user messages from extensions.
 * Unlike ego.sendMessage() which sends custom messages, sendUserMessage() sends
...
export default function (ego: ExtensionAPI) {
	...
	ego.registerCommand("ask", {
		...
		handler: async (args, ctx) => {
			...
			ego.sendUserMessage(args);
		},
	});

	// Command that steers the agent mid-conversation
	ego.registerCommand("steer", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				ego.sendUserMessage(args, { deliverAs: "steer" });
			}
		},
	});

	// Command that queues a follow-up message
	ego.registerCommand("followup", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				ego.sendUserMessage(args, { deliverAs: "followUp" });
