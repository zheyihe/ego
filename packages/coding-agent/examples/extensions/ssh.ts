 ...
 *
 * Usage:
 *   ego -e ./ssh.ts --ssh user@host
 *   ego -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
export default function (ego: ExtensionAPI) {
	ego.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	ego.on("session_start", async (_event, ctx) => {
		...
		const arg = ego.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
