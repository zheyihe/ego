...
Example extensions for ego-coding-agent.

## Usage

```bash
# Load an extension with --extension flag
ego --extension examples/extensions/permission-gate.ts

# Or copy to extensions directory for auto-discovery
cp permission-gate.ts ~/.ego/agent/extensions/
```
...
| `send-user-message.ts` | Demonstrates `ego.sendUserMessage()` for sending user messages from extensions |
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs ego theme with macOS dark/light mode |
...
| `event-bus.ts` | Inter-extension communication via `ego.events` |
...
| `custom-provider-gitlab-duo/` | GitLab Duo provider using ego-ai's built-in Anthropic/OpenAI streaming via proxy |
