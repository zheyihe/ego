...
export default function (ego: ExtensionAPI) {
	ego.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await ego.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[ego] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await ego.exec("git", ["add", "-A"]);
		const { code: commitCode } = await ego.exec("git", ["commit", "-m", commitMessage]);
