...
import type { ExtensionAPI, SessionBeforeSwitchEvent, SessionMessageEntry } from "@zheyihe/ego-coding-agent";

export default function (ego: ExtensionAPI) {
	ego.on("session_before_switch", async (event: SessionBeforeSwitchEvent, ctx) => {
		if (!ctx.hasUI) return;
