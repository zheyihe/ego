...
import { getModel, type OAuthCredentials, type OAuthProvider } from "@zheyihe/ego-ai";
...
// ============================================================================
// OAuth API key resolution from ~/.ego/agent/auth.json
// ============================================================================

const AUTH_PATH = join(homedir(), ".ego", "agent", "auth.json");
...
/**
 * Resolve API key for a provider from ~/.ego/agent/auth.json
 *
...
/**
 * Check if a provider has credentials in ~/.ego/agent/auth.json
 */
...
/** Path to the real ego agent config directory */
export const EGO_AGENT_DIR = join(homedir(), ".ego", "agent");

/**
 * Get an AuthStorage instance backed by ~/.ego/agent/auth.json
...
export function createTestSession(options: TestSessionOptions = {}): TestSessionContext {
	const tempDir = join(tmpdir(), `ego-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
