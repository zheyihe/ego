...
describe("AgentSession concurrent prompt guard", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `ego-concurrent-test-${Date.now()}`);
	...
	it("should queue extension-origin steering messages while streaming", async () => {
		...
		const extensionsResult = await createTestExtensionsResult([
			(ego) => {
				(globalThis as typeof globalThis & { testExtensionApi?: unknown }).testExtensionApi = ego;
			},
			(ego) => {
		...
		ego!.sendUserMessage("Steer from extension", { deliverAs: "steer" });
