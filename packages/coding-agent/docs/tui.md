> ego can create TUI components. Ask it to build one for your use case.

# TUI Components

Extensions and custom tools can render custom TUI components for interactive user interfaces. This page covers the component system and available building blocks.

**Source:** [`@zheyihe/ego-tui`](https://github.com/zheyihe/ego/tree/main/packages/tui)
...
**In custom tools** via `ego.ui.custom()`:

```typescript
async execute(toolCallId, params, onUpdate, ctx, signal) {
  const handle = ego.ui.custom(myComponent);
  // ...
  handle.close();
}
```
...
Import from `@zheyihe/ego-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@zheyihe/ego-tui";
```
...
```typescript
import { getMarkdownTheme } from "@zheyihe/ego-coding-agent";
import { Markdown } from "@zheyihe/ego-tui";
...
```bash
EGO_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `@zheyihe/ego-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@zheyihe/ego-coding-agent";
import { DynamicBorder } from "@zheyihe/ego-coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@zheyihe/ego-tui";
...
```typescript
import { BorderedLoader } from "@zheyihe/ego-coding-agent";

ego.registerCommand("fetch", {
  handler: async (_args, ctx) => {
    const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
...
For toggling multiple settings. Use `SettingsList` from `@zheyihe/ego-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@zheyihe/ego-coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@zheyihe/ego-tui";

ego.registerCommand("settings", {
  handler: async (_args, ctx) => {
...
Customize the inline working indicator shown while ego is streaming a response.
...
import { matchesKey, truncateToWidth } from "@zheyihe/ego-tui";
