> ego can create themes. Ask it to build one for your setup.
...
Ego loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.ego/agent/themes/*.json`
- Project: `.ego/themes/*.json`
- Packages: `themes/` directories or `ego.themes` entries in `package.json`
...
On first run, ego detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.ego/agent/themes
vim ~/.ego/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/zheyihe/ego/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, ego reloads it automatically for immediate visual feedback.
...
Ego uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, ego falls back to the nearest approximation.
